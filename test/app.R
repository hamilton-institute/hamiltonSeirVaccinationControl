...
ui <- fluidPage(
    ...
    sidebarLayout(
        column(
            width = 3,
            dateInput(("start_vac"), "Starting date of vaccination", "2021-01-01" ),
            sliderInput(("vacc_control"), "Percentage of population to vaccinate (%)", 0, 100, 60, step = 1),
        ...
        mainPanel(
            plotly::plotlyOutput(("plot"), height = 500),
...
server <- function(input, output, session) {
    ...
    output$plot <- plotly::renderPlotly({
        ...
        final_raw$Date = seq.Date(from = input$start_date, to = input$start_date + nrow(final_raw) - 1,
                                  by = 1)
